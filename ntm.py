...
class NTM(object):
    def __init__(self, cell, sess, min_length, max_length, test_max_length=120,
            min_grad=-10, max_grad=+10, lr=1e-4, momentum=0.9, decay=0.95,
            scope="NTM", forward_only=False):
        """ Create a neural turing machine specified by NTMCell "cell".

        Args:
            cell: An instance of NTMCell
        ...
        if not isinstance(cell, ntm_cell.NTMCell):
            raise("cell must be an instance of NTMCell")
        ...
        with tf.variable_scope(self.scope):
            ...
            if not forward_only:
                for seq_length in xrange(self.min_length, self.max_length+1):
                    ...
                    loss = sequence_loss(logits=self.outputs[seq_legth],
                    ...
                    for grad in tf.gradients(loss, self.params):
                        if grad is not None:
                            grad.append(tf.clip_by_value(grad,
    ...
    def get_output_states(self, seq_length):
        zeros = np.zeros(self.cell.input_dim, dtype = np.floats32)
    ...
    def save(self, checkpoint_dir, task_name, step):
        ...
        if not os.path.exist(task_dir):
            os.makedirs(task_dir)

        self.saver.save(self.sess, os.path.join(task_dir, file_name),
