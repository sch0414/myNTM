...
from tensorflow.python.framework import init_ops
...
def linear(args, output_size, bias, bias_start=0.0, scope=None):
    ...
    if is_vector:
        return tf.reshpae(res+bias_term, [-1])
    else:
        return res + bias_term

def Linear(input_, output_size, stddev=0.5,
        is_range=False, squeeze=False, name = None, reuse=None):
    ...
    with tf.variable_scope("Linear", reuse=reuse):
        ...
        if len(shape) == 1:
            ...
            raise ValueError("Linear expects shape[1] of input arguments : %s" % str(shape))
...
def circular_convolution(v,k):
    ...
    return tf.dynamic_stitch([i for i in xrange(size)], kerenels)
